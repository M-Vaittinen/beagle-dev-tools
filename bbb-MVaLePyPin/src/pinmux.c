...
void pinmux_print_modes(const struct pin_info *pin)
{
	...
	for (i = 0; i < NUM_MODES; i++) {
		const char *sig =
			pin->mode[i] ? pin->mode[i] : "(not available)";
		const char *note = "";
		if (i == 7)
...
void pinmux_print_reg(const struct pin_info *pin)
{
	...
	printf("  Physical address: 0x%08X  (Control Module base 0x%08X + "
	       "offset 0x%03X)\n",
	...
	printf("  [5]     0    RXACTIVE     Input buffer disabled "
	       "(output-only)\n");
	printf("  [5]     1    RXACTIVE     Input buffer enabled (required for "
	       "inputs)\n");
	...
	printf("  [2:0]  0-7   MUXMODE      Selects MODE0..MODE7 (MODE7 = "
	       "GPIO)\n");
	printf("\n");
	...
	printf("    0x27  GPIO input  (MODE7, pull-down enabled, input buf "
	       "on)\n");
	printf("    0x2F  GPIO input  (MODE7, pull disabled,     input buf "
	       "on)\n");
	printf("    0x37  GPIO input  (MODE7, pull-up enabled,   input buf "
	       "on)\n");
}
