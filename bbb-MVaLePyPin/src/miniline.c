...
static int utf8_seq_len(unsigned char c)
{
	if (c < 0x80)
		return 1;
	if (c < 0xC0)
		return 1; /* unexpected continuation byte — treat as 1 */
	if (c < 0xE0)
		return 2;
	if (c < 0xF0)
		return 3;
...
static int utf8_prev_char_len(const char *buf, int pos)
{
	...
	while (pos > 0) {
		pos--;
		len++;
		/* A UTF-8 sequence start byte is either < 0x80 (ASCII) or >=
		 * 0xC0 */
		if ((unsigned char)buf[pos] < 0x80 ||
			(unsigned char)buf[pos] >= 0xC0)
			break;
	}
	return len;
}
...
static const char *hist_get(int idx)
{
	...
	if (g_hist_count == 0 || idx < 0 || idx >= g_hist_count)
		return NULL;
	oldest =
		(g_hist_count > ML_HIST_SIZE) ? g_hist_count - ML_HIST_SIZE : 0;
	if (idx < oldest)
		return NULL;
		...
		if (c >= 0xC0) {
			...
			for (i = 1; i < slen; i++) {
				...
				if (read(STDIN_FILENO, &cont, 1) <= 0)
					return KEY_UNKNOWN;
				/* Validate continuation byte (should be
				 * 0x80–0xBF) */
				if ((cont & 0xC0) != 0x80)
					return KEY_UNKNOWN;
				mb[i] = (char)cont;
...
char *miniline(const char *prompt)
{
	...
	for (;;) {
		...
		case KEY_END:
			if (pos < len) {
				cursor_right(
					utf8_disp_width(buf + pos, len - pos));
				pos = len;
			}
			break;
		...
		case KEY_RIGHT:
			if (pos < len) {
				int clen =
					utf8_seq_len((unsigned char)buf[pos]);
				cursor_right(1);
				pos += clen;
			}
			break;
