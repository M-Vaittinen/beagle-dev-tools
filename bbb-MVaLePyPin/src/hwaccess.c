...
static const mem_region_t allowed_regions[] = {
...
static mapped_page_t g_pages[MAX_MAPPED_PAGES];
...
void hw_close(void)
{
    ...
    if (g_kmod_fd   >= 0) { close(g_kmod_fd);   g_kmod_fd   = -1; }
    g_available = 0;
    g_method    = "none";
}

int hw_available(void) { return g_available; }

const char *hw_method(void) { return g_method; }
...
int hw_read32(uint32_t phys_addr, uint32_t *value)
{
    ...
    if (!addr_allowed(phys_addr)) {
        fprintf(stderr, "hwaccess: address 0x%08X not in allowed range\n", phys_addr);
        return -1;
    }
    ...
    if (g_devmem_fd >= 0) {
        ...
        if (!page) {
            fprintf(stderr, "hwaccess: mmap failed for 0x%08X: %s\n",
