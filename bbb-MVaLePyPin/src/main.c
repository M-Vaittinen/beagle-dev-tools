...
{
	const struct pin_info *pin = pin_find(pin_name);
	if (!pin) {
		printf("  Unknown pin '%s'. Try 'list' to see available "
		       "pins.\n",
			pin_name);
		return;
	}
...
static void cmd_list(const char *connector)
{
	...
	if (show_p8) {
		printf("  %-8s  %-6s  %-24s  %-12s  %-5s  %s\n", "Pin", "Ball",
			"Pad (Mode 0)", "GPIO", "sysfs", "Default");
		printf("  %-8s  %-6s  %-24s  %-12s  %-5s  %s\n", "--------",
			"------", "------------------------", "------------",
			"-----", "-------");
		for (i = 0; i < p8_pin_count; i++) {
			const struct pin_info *p = &p8_pins[i];
			char gpiostr[24] = "-";
			char sysfsstr[8] = "-";
			if (p->gpio_bank >= 0) {
			...
			printf("  %-8s  %-6s  %-24s  %-12s  %-5s  %s\n",
				p->name, p->ball, p->mode[0] ? p->mode[0] : "-",
				gpiostr, sysfsstr,
				p->mode[7] ? p->mode[7] : "-");
		}
		if (show_p9)
	...
	if (show_p9) {
		printf("  %-8s  %-6s  %-28s  %-12s  %-5s  %s\n", "Pin", "Ball",
			"Pad (Mode 0)", "GPIO", "sysfs", "GPIO (Mode7)");
		printf("  %-8s  %-6s  %-28s  %-12s  %-5s  %s\n", "--------",
			"------", "----------------------------",
			"------------", "-----", "------------");
		for (i = 0; i < p9_pin_count; i++) {
			const struct pin_info *p = &p9_pins[i];
			char gpiostr[24] = "-";
			char sysfsstr[8] = "-";
			if (p->gpio_bank >= 0) {
				snprintf(gpiostr, sizeof(gpiostr), "GPIO%d[%d]",
...
#define HW_NOT_AVAIL_MSG                                                       \
	...
	"  Run on a BeagleBone Black with /dev/mem or the bbb_pinctrl kernel " \
	"module.\n"
...
static void cmd_read_mux(const char *pin_name)
{
	...
	if (hw_read32(phys, &val) < 0)
		return;

	printf("  Pinmux register for %s (%s @ 0x%08X):\n", pin->name,
		pin->conf_reg_name, phys);
...
static void cmd_write_mux(const char *pin_name, int mode)
{
	...
	printf("  Writing 0x%08X -> %s @ 0x%08X  (was 0x%08X)\n", nval,
		pin->conf_reg_name, phys, cur);

	if (hw_write32(phys, nval) < 0)
		return;
...
static void cmd_read_gpio(const char *pin_name)
{
	...
	if (hw_read32(base + GPIO_OE, &oe_val) < 0)
		return;
	if (hw_read32(base + GPIO_DATAIN, &datain_val) < 0)
		return;
	if (hw_read32(base + GPIO_DATAOUT, &dataout_val) < 0)
		return;

	gpio_print_state(pin, oe_val, datain_val, dataout_val);

	printf("    Raw OE[%d]      = %d  (full reg: 0x%08X)\n", pin->gpio_bit,
		(oe_val & mask) ? 1 : 0, oe_val);
	printf("    Raw DATAIN[%d]  = %d  (full reg: 0x%08X)\n", pin->gpio_bit,
		(datain_val & mask) ? 1 : 0, datain_val);
	printf("    Raw DATAOUT[%d] = %d  (full reg: 0x%08X)\n", pin->gpio_bit,
		(dataout_val & mask) ? 1 : 0, dataout_val);
}
...
static void cmd_write_gpio_dir(const char *pin_name, int is_input)
{
	...
	printf("  Setting GPIO%d[%d] direction: %s\n", pin->gpio_bank,
		pin->gpio_bit, is_input ? "INPUT" : "OUTPUT");
	if (hw_write32(base + GPIO_OE, oe_val) < 0)
		return;
	printf("  Done.\n");
}
...
{
	printf("  BeagleBone Black Pinmux/GPIO Shell\n"
	       "  ----------------------------------\n"
	       "  All pin names are case-insensitive (e.g. P8_03, p8_03, "
	       "P9_15).\n"
	       "\n"
	       "  Information commands (available on PC and BBB):\n"
	       "    help                     Show this help\n"
	       "    info  <pin>              Full pin summary (modes, "
	       "registers, GPIO)\n"
	       "    mux   <pin>              Mux mode table (MODE0..MODE7)\n"
	       "    reg   <pin>              Pinmux register address and "
	       "bitfield\n"
	       "    gpio  <pin>              GPIO bank/bit, sysfs number, "
	       "register addresses\n"
	       "    list  [P8|P9]            List all pins (optional: filter "
	       "to one connector)\n"
	       "\n"
	       "  Hardware commands (BBB only - require /dev/mem or "
	       "bbb_pinctrl kmod):\n"
	       "    read mux   <pin>         Read current pinmux setting from "
	       "hardware\n"
	       "    write mux  <pin> <mode>  Set MUXMODE field (0-7) leaving "
	       "other bits\n"
	       "    write mux  <pin> raw <hex>  Write raw value to pinmux "
	       "register\n"
	       "    read gpio  <pin>         Read current GPIO direction and "
	       "value\n"
	       "    write gpio <pin> <0|1>   Write GPIO output value (1=high, "
	       "0=low)\n"
	       "    write gpio <pin> <in|out>  Set GPIO direction\n"
	       "    readreg   <hex_addr>     Read 32-bit register at physical "
	       "address\n"
	       "    writereg  <hex_addr> <hex_val>  Write register at physical "
	       "address\n"
	       "\n"
	       "  Examples:\n"
	       "    info P9_15\n"
...
static void cmd_easter_egg(void)
{
	...
	/* Clear screen and return cursor to top-left; shell loop reprints
	 * prompt */
	printf("\x1b[2J\x1b[H");
	fflush(stdout);
}
...
static int dispatch(char *line)
{
	...
	if (strcmp(cmd, "write") == 0) {
		if (!arg1 || !arg2) {
			printf("  Usage: write mux <pin> <mode>  |  write gpio "
			       "<pin> <val>\n");
			return 0;
		}
		if (strcmp(arg1, "mux") == 0) {
			if (!arg3) {
				printf("  Usage: write mux <pin> <mode 0-7>  | "
				       " write mux <pin> raw <hex>\n");
				return 0;
			}
			if (strcmp(arg2, "raw") == 0) {
				/* write mux raw <pin> <hex> - reparse: arg2 is
				 * pin if arg1=mux */
				/* Actually: write mux <pin> raw <hex_val> */
				/* arg2 = pin, arg3 = "raw" or a number ... */
				/* Re-check: cmd="write" arg1="mux" arg2=<pin>
				 * arg3=<mode_or_"raw"> */
				/* If arg3 == "raw": need one more token -- not
				 * captured; handle differently */
				printf("  Usage: write mux <pin> raw "
				       "<hex_val>\n");
				printf("  (The 5-token form requires typing "
				       "the full command)\n");
				return 0;
			}
			{
		...
		if (strcmp(arg1, "gpio") == 0) {
			if (!arg3) {
				printf("  Usage: write gpio <pin> <0|1>  |  "
				       "write gpio <pin> <in|out>\n");
				return 0;
			}
			if (strcmp(arg3, "in") == 0) {
				cmd_write_gpio_dir(arg2, 1);
				return 0;
			}
			if (strcmp(arg3, "out") == 0) {
				cmd_write_gpio_dir(arg2, 0);
				return 0;
			}
			{
				char *endp;
...
#define LCOPY(dst, src)                                                        \
	do {                                                                   \
		strncpy(dst, src, sizeof(dst) - 1);                            \
		dst[sizeof(dst) - 1] = '\0';                                   \
		for (char *_p = dst; *_p; _p++)                                \
			*_p = (char)tolower(*_p);                              \
	} while (0)
			LCOPY(lc0, tok[0]);
			LCOPY(lc1, tok[1]);
			...
			if (strcmp(lc0, "write") == 0 &&
				strcmp(lc1, "mux") == 0 &&
				strcmp(lc3, "raw") == 0) {
				char *endp;
				uint32_t raw =
					(uint32_t)strtoul(tok[4], &endp, 0);
				if (!*endp) {
					cmd_write_mux_raw(tok[2], raw);
					return 0;
				}
			}
		}
...
static void print_banner(void)
{
	...
	if (detect_is_bbb()) {
		if (hw_available())
			...
			printf("HW access : unavailable (try running as "
			       "root)\n");
	} else {
