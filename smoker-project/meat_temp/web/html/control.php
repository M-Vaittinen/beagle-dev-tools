...
<html xmlns="http://www.w3.org/1999/xhtml">
  ...
  <body>
  <a href="index.php">Status</a><br><br>
  <h1>Target temperatures</h1>
<table>
 <tr>
  <td>
  <?php
	...
	echo "Smoker target $target_temp";
?>
  </td>
  <td>
 ...
 <tr>
  ...
  <?php
	...
	echo "Food target $meat_target_temp";
?>
  </td>
  <td>
  <form action="<?php echo htmlentities($_SERVER['PHP_SELF']);?>" method="post">
...
<?php
	...
	if ($warming == 1) {
		$text = "Heating...";
		$new_state='0';
		$new_state_text='stop heating';
	}
	else
	{
		$text = "Not-heating";
		$new_state='1';
		$new_state_text='start heating';
	}
	echo "<h3>$text</h3>";
...
<?php
	...
	if ($forced == 0) {
		$text = "AUTO mode (keep at target temp)";
		$new_state='1';
		$new_state_text='switch to forced (discard target temp)';
	}
	else
	{
		$text = "FORCED mode (ignoring target temp)";
		$new_state='0';
		$new_state_text='switch to auto (try reach target temp)';
	}
	echo "<h3>$text</h3>";
